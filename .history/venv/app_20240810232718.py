...
first_card = [
    ...
    dbc.CardBody( id = "sales_card", className="card-text"),
        ],
second_card = [
...
@app.callback(
    Output('sub-categories_dropdown', 'options'),
...
@app.callback(
    ...
    [Input('bikeshop_dropdown', 'value')]


def update_cards(selected_bikeshop):
    # Filter the DataFrame based on the selected bikeshop
    filtered_df = df.loc[selected_bikeshop]
    
    # Get the values for total_price and quantity
