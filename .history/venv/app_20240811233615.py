...
def update_cards(total_clicks, mountain_clicks, road_clicks, selected_subcategory):
    ...
    elif triggered_id == 'Total-option':
        filtered_df = df[df.index.isin(['Mountain', 'Road'])]

    # Sum total_price and quantity
    total_price = df[df.index.isin(['Mountain', 'Road'])]['total_price'].sum()
    quantity = df[df.index.isin(['Mountain', 'Road'])]['total_price'].sum()
