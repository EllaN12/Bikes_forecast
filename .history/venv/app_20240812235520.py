...
dropdown2 = dbc.DropdownMenu(
    ...
    label="Total",
    children=[
        dbc.DropdownMenuItem("Select a Sub-Category", id="sub_category_placeholder")
    ]
)
...
app.layout = dbc.Container([
    ...
    dbc.Row([html.Div(
                    id="intro",
                    children= "This dashboard offers a review of FY2015 sales data and a Q1 2016 forecast across various bike categories, sub-categories, and store locations",
                    html.Hr(),
                )]),
    dbc.Row([
        dbc.Col([
...
@app.callback(
    Output('sub_categories_dropdown', 'children'),
...
def update_sub_categories(selected_category):
    if selected_category == 'Mountain':
        ...
        options = [{'label': 'Total', 'value': 'Total'}]  # Return "Total" by default
    
    return [dbc.DropdownMenuItem(option['label'], id={'type': 'sub_category_option', 'index': option['value']}) for option in options]
...
def plot_forecast_line(overview_clicks, mountain_clicks, road_clicks, bikeshops_clicks):
    ...
    try:
        ...
        if clicked_id == "Mountain-option":
            ...
            df = forecast_data_pull(Road = True)
