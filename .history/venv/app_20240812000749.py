...
def update_bikeshops_bar_chart(overview_clicks, mountain_clicks, road_clicks, bikeshops_clicks, sub-categories_dropdown ):
    try:
        ctx = dash.callback_context
        if not ctx.triggered:
            return no_update
...
def make_forecast_graphs(mountain_clicks, road_clicks, bikeshops_clicks, sub-categories_dropdown ):
    try:
        ctx = dash.callback_context
        if not ctx.triggered:
            return no_update
